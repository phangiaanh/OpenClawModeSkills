...
if MW_MARKER in esrc:
    ...
    if ANCHOR:
        ...
        INJECT_MW = (
            ...
            '\t\t\tconst _out = JSON.parse(_es("python3", [_ENGINE, "handle-callback", _d, "--chat-id", String(ctx.callbackQuery.message?.chat.id ?? ""), "--message-id", String(ctx.callbackQuery.message?.message_id ?? "")], { timeout: 8000 }).toString().trim());\n'
            '\t\t\tif (!_out.error) {\n'
            '\t\t\t\ttry {\n'
...
if CB_MARKER in esrc:
    ...
    if CB_ANCHOR not in esrc:
        ...
        INJECT_CB = (
            ...
            '\t\t\t\t\tconst _cbRaw = _cbes("python3", ["' + ENGINE_PATH + '", "handle-callback", data, "--chat-id", String(ctx.callbackQuery.message?.chat.id ?? ""), "--message-id", String(ctx.callbackQuery.message?.message_id ?? "")], { encoding: "utf-8", timeout: 10000 });\n'
            '\t\t\t\t\tconst _cbOut = JSON.parse(_cbRaw);\n'
            '\t\t\t\t\tif (_cbOut && !_cbOut.error && (_cbOut.text || _cbOut.toast !== undefined)) {\n'
            '\t\t\t\t\t\tif (_cbOut.toast !== undefined) { return; }\n'
